...
// Patch feature gates (~/.clawgod/patches.json + CLAWGOD_FEATURES env) —
...
// ({"<featureId>": false}) or the CLAWGOD_FEATURES env override
...
// reads patches.json + CLAWGOD_FEATURES) and decides the ON/OFF of each
// baked-in gate. Gate checks inside replacers land in the follow-up commit.
